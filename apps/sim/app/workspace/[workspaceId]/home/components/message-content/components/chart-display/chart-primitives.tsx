/**
 * HyperFix chat-light (Phase B2) : primitives recharts (conteneur, tooltip,
 * légende), portées depuis nao (`apps/frontend/src/components/ui/chart.tsx`,
 * shadcn). Seul l'import `cn` est adapté.
 */
...
import { formatPercentShare, sumPercentStackBase } from '@/lib/charts/nao/chart-builder'
import { CHART_NUMBER_LOCALE, formatChartValue } from '@/lib/charts/nao/chart-values'
import type * as displayChart from '@/lib/charts/nao/display-chart'
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      ...
      >
        ...
        <div
          className={cn(
            // biome-ignore format: single string with inner quotes (nao original) — the formatter breaks them
            // biome-ignore lint/nursery/useSortedClasses: keep nao class order (sorter breaks inner quotes)
  ...
  return (
    ...
    >
      ...
      <div className='grid gap-1.5'>
        ...
        {showTotal && (
          <div className='mt-0.5 flex w-full items-center gap-2 border-border/50 border-t pt-1.5'>
        ...
        .map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                'flex shrink-0 select-none items-center gap-1.5 whitespace-nowrap text-muted-foreground [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground',
