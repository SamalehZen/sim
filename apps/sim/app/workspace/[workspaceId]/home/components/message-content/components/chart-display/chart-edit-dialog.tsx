/**
 * HyperFix chat-light (Phase B2) : dialogue d'édition des graphiques, porté
 * depuis nao (`apps/frontend/src/components/tool-calls/display-chart-edit-dialog.tsx`,
 * partie `ChartConfigEditDialog` + aides). ADAPTATIONS : imports sim, selects
 * natifs (FieldSelect), dialogue Radix local, icônes `@sim/emcn`.
 */

import { useEffect, useMemo, useState } from 'react'
import { Button, cn, Input, Switch } from '@sim/emcn'
import { ChartColumn, Plus, Trash, X } from '@sim/emcn/icons'
import { computeKpiComparison, DEFAULT_COLORS } from '@/lib/charts/nao/chart-builder'
...
const X_AXIS_TYPE_OPTIONS: {
  value: NonNullable<displayChart.XAxisType> | 'auto'
  label: string
}[] = [
...
export type EditableChartInput = Omit<displayChart.KpiCardInput, 'chart_type'> & {
...
export function ChartConfigEditDialog({
  ...
  const canShowComparisonPill = useMemo(
    () =>
      ...
      hasRenderableKpiComparison(data, draft.x_axis_key, draft.series),
  ...
  const updateSeriesValueFormatAt = (
    index: number,
    field: 'd3_format' | 'prefix' | 'suffix',
    value: string
  ) => {
  ...
  const setSeriesUnit = (
    index: number,
    { unit, placement }: { unit: string; placement: UnitPlacement }
  ) => {
    const series = draft.series[index]
  ...
  const addSeries = () => {
    ...
    const fallback =
      selectableColumns.find((c) => c !== draft.x_axis_key && !used.has(c)) ?? selectableColumns[0]
    if (!fallback) {
...
function ColumnSelect({ value, columns, onChange }: ColumnSelectProps) {
  ...
  const items =
    value && !columnsWithValues.includes(value) ? [value, ...columnsWithValues] : columnsWithValues
  return (
    <FieldSelect
      value={value}
      ...
      options={[
        ...
        ...items.map((column) => ({ value: column, label: column })),
      ]}
    />
  )
}
...
function AxisFields({
  ...
  return (
    <div
      className={`grid gap-3 items-end ${showRange ? 'grid-cols-[2fr_1fr_1fr]' : 'grid-cols-1'}`}
    >
      <div className='grid gap-1'>
...
function SeriesTypeSelect({ value, onChange }: SeriesTypeSelectProps) {
  return (
    <FieldSelect
      value={value}
      ariaLabel='Series type'
      onChange={(next) => onChange(next as displayChart.SeriesType)}
  ...
  return (
    <div className='grid grid-cols-[1fr_1fr_8rem] gap-2'>
      ...
      <div className='grid gap-1'>
        ...
        <FieldSelect
          value={placement}
          ariaLabel='Unit placement'
          onChange={(value) => onUnitChange(unit, value as UnitPlacement)}
          options={[
  ...
  return (
    ...
    >
      {unit ? (
        ...
        <span className='max-w-full truncate px-0.5 text-xs font-semibold text-muted-foreground'>
          $
        </span>
      )}
    </Button>
...
function YAxisSideToggle({ value, onChange }: YAxisSideToggleProps) {
  ...
  return (
    <Button
      ...
      title={isRight ? 'Right Y-axis' : 'Left Y-axis'}
      onClick={() => {
        onChange(isRight ? 'left' : 'right')
      }}
    >
      <ChartColumn className={`size-4 transition-transform ${isRight ? '-rotate-90' : ''}`} />
    </Button>
  )
}
...
function hasRenderableKpiComparison(
  ...
  return series.some(
    (s) => computeKpiComparison(data, xAxisKey ?? '', s.data_key, 'absolute') != null
  )
}
