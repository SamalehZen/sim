...
export function isConditionalFormatRule(value: unknown): value is ConditionalFormatRule {
  ...
  if (rule.type === 'string') {
    if (
      !STRING_OPERATORS.includes(rule.operator as StringOperator) ||
      typeof rule.color !== 'string'
    ) {
      return false
    }
...
function scaleEndpoints(rule: ColorScaleRule): { minColor: string; maxColor: string } {
  ...
  return {
    minColor:
      rule.minColor ?? (derived ? toRgbaString(derived, SCALE_MIN_ALPHA) : DEFAULT_SCALE_MIN_COLOR),
    maxColor:
      rule.maxColor ?? (derived ? toRgbaString(derived, SCALE_MAX_ALPHA) : DEFAULT_SCALE_MAX_COLOR),
  }
}
