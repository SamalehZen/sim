...
export function computeValueAxisWidth(
  ...
  const maximumLabelLength = Math.max(
    ...candidates.map((value) => formatValueYAxisTick(value, valueFormat).length)
  )
  const estimatedWidth =
    maximumLabelLength * VALUE_AXIS_CHARACTER_WIDTH +
    VALUE_AXIS_PADDING +
    labelAllowance +
    labelAllowance
  return Math.min(
...
export function computeKpiComparison(
  ...
  const periodLabel =
    dateKey == null
      ? 'previous period'
      : describePreviousPeriod(previousRow?.[dateKey], currentRow?.[dateKey])
  ...
  if (mode === 'percentage') {
    ...
    return {
      valueText: formatPercentMagnitude(Math.abs(pct)),
      direction,
      colored: true,
      periodLabel,
    }
  ...
  if (mode === 'variation') {
    return {
      valueText: formatCompactNumber(Math.abs(delta)),
      direction,
      colored: true,
      periodLabel,
    }
...
export function percentStackSeries(
  series: displayChart.SeriesConfig[]
...
function KpiCard({
  ...
  const showArrowAndColor = comparison?.colored && comparison.direction !== 'flat'
  ...
  return (
    <div className='min-w-[160px]'>
      ...
      <div className='font-medium text-3xl tabular-nums'>{formattedValue}</div>
      {comparison && (
        <div
          className={`mt-1.5 flex items-center gap-1.5 whitespace-nowrap text-sm ${pillColorClass}`}
        >
...
function buildBarChart(props: ResolvedProps) {
  ...
  const axisValues = isStacked
    ? collectStackedAxisValues(data, dataKeys)
  ...
  const valueAxisValues =
    typeof yAxisDomain?.[1] === 'number' ? [...axisValues, yAxisDomain[1]] : axisValues
  const valueAxisWidth = computeValueAxisWidth(
    valueAxisValues,
    chartLevelFormat,
    Boolean(yAxisLabel)
  )
    ...
    >
      ...
      {renderedSeries.map((s, i) => (
        <Bar
          ...
          shape={
            isStacked ? renderStackedBarShape(seriesKeys, s.data_key, separatorColor) : undefined
          }
 ...
 */
export function isTopmostStackSegment(
...
function buildComboChart(props: ResolvedProps) {
  ...
  const rightAxisWidth = computeComboAxisWidth(
    data,
    rightSeries,
    rightFormat,
    Boolean(yAxisRightLabel)
  )
...
function buildPieChart(props: ResolvedProps) {
  ...
  const dataLabelsLayer = showDataLabels
    ? renderPieDataLabelsLayer(series[0].value_format)
    : undefined
...
export function bucketPieData(
  ...
  if (existingOtherIndex !== -1) {
    ...
    merged[existingOtherIndex] = {
      ...existing,
      [valueKey]: toNumericValue(existing[valueKey]) + otherValue,
    }
    return merged
  }
