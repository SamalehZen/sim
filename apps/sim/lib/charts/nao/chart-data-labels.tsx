...
export function shouldReserveDataLabelHeadroom<Props extends DataLabelChartProps>(
  props: Props
): boolean {
...
export function shouldReserveStackTotalFootroom<Props extends DataLabelChartProps>(
  props: Props
): boolean {
...
export function renderDataLabelsLayer(series: displayChart.SeriesConfig[]) {
  return createLabelsLayer(
    'DataLabelsLayer',
    ({ formattedGraphicalItems, offset, width, height }) =>
      collectLabelCandidates(formattedGraphicalItems ?? [], offset ?? {}, width, height, series)
  )
}

export function renderStackTotalLabelsLayer(
  data: Record<string, unknown>[],
  series: displayChart.SeriesConfig[]
) {
  const valueFormat = getChartLevelValueFormat(series)
  return createLabelsLayer(
    'StackTotalLabelsLayer',
    ({ formattedGraphicalItems, offset, width, height }) => {
...
export function renderPieDataLabelsLayer(valueFormat?: displayChart.ValueFormat) {
  return createLabelsLayer('PieDataLabelsLayer', ({ formattedGraphicalItems, width, height }) => {
    const pie = (formattedGraphicalItems ?? []).find(
      (entry) => entry.item?.type?.displayName === 'Pie'
    )
    return collectPieLabelCandidates(pie?.props?.sectors ?? [], width, height, valueFormat)
  })
}

export function getDataLabelSetup<
  Props extends Pick<DataLabelChartProps, 'data' | 'series' | 'showDataLabels'>,
>(props: Props, isStacked: boolean) {
...
function stackTotalAnchor(
  ...
  items.forEach((item) => {
    ...
    if (
      extreme == null ||
      (isPositive ? anchor.anchorY < extreme.anchorY : anchor.anchorY > extreme.anchorY)
    ) {
      extreme = anchor
    }
  })
...
function collectLabelCandidates(
  ...
  const graphicalItems = items.filter((entry) =>
    LABELLED_SERIES_KINDS.has(entry.item?.type?.displayName ?? '')
  )
  return graphicalItems.flatMap((entry, seriesIndex) => {
    ...
    const seriesConfig =
      series.find((item) => dataKey != null && item.data_key === String(dataKey)) ??
      series[seriesIndex]
...
function selectSeriesLabelCandidates(
  ...
  const minimum = candidates.reduce((best, current) =>
    current.value < best.value ? current : best
  )
  const maximum = candidates.reduce((best, current) =>
    current.value > best.value ? current : best
  )
  ...
  const selectedExtrema =
    extrema.length <= remainingSlots()
      ? extrema
      : sampleLabelCandidatesEvenly(extrema, remainingSlots())
  ...
  sampleLabelCandidatesEvenly(remaining, remainingSlots()).forEach((entry) =>
    selectedIndices.add(entry.index)
  )
  return candidates.filter((entry) => selectedIndices.has(entry.index))
}

function sampleLabelCandidatesEvenly(
  candidates: IndexedLabelCandidate[],
  count: number
): IndexedLabelCandidate[] {
  if (count <= 0) {
  ...
  const ordered = [...candidates].sort(
    (a, b) => a.candidate.cx - b.candidate.cx || a.index - b.index
  )
...
function labelAnchor(
  ...
  value: number,
  isBar: boolean
): { cx: number; baselineY: number } | null {
...
function labelBox(
  ...
  const left =
    textAnchor === 'start' ? cx : textAnchor === 'end' ? cx - halfWidth * 2 : cx - halfWidth
  const right =
    textAnchor === 'start' ? cx + halfWidth * 2 : textAnchor === 'end' ? cx : cx + halfWidth
  return {
