/**
 * HyperFix chat-light (Phase B2) : assemblage SVG + rasterisation PNG,
 * porté depuis nao (`apps/backend/src/utils/generate-chart.ts`).
 */
...
function createSvgWithRightLegend(
  html: string,
  width: number,
  height: number,
  legend: LegendEntry[]
...
function buildLegend(entries: LegendEntry[], width: number, centerY: number): string {
  ...
  const totalWidth = entries.reduce(
    (sum, e, i) =>
      ...
      swatchSize +
      gap +
      e.label.length * charWidth +
      (i < entries.length - 1 ? itemSpacing : 0),
    0
  )
