/**
 * HyperFix chat-light (Phase B2) : schéma de l'outil `display_chart`, porté
 * depuis nao (`apps/shared/src/tools/display-chart.ts`).
 *
 * ADAPTATION : nao lit les données via `query_id` (sortie d'un `execute_sql`
 * précédent). Ici les données viennent des tables Sim en lecture live, donc
 * `query_id` est remplacé par `source` (table liée ou lignes statiques lues
 * par Luna via ses outils). Tout le reste est identique à nao.
 */
...
export const ChartTableSourceSchema = z.object({
  type: z.literal('table'),
  tableId: z.string().describe('Sim table id to read live.'),
  filter: z
    .unknown()
    .describe('Mongo-style filter object (same shape as the table query tool).')
    .optional(),
  sort: z.unknown().describe('Sort object, e.g. {"score": "desc"}.').optional(),
  limit: z.number().describe('Max rows to read.').optional(),
})

export const ChartStaticSourceSchema = z.object({
  type: z.literal('static'),
  rows: z
    .array(z.record(z.string(), z.unknown()))
    .describe('Rows already read through the table tools (column names as keys).')
    .optional(),
})

export const ChartSourceSchema = z.union([ChartTableSourceSchema, ChartStaticSourceSchema])
...
const ChartTypeNameSchema = z
  ...
  .regex(
    /^[a-z][a-z0-9_-]*$/,
    'Chart type must use lowercase letters, numbers, underscores, or hyphens.'
  )
...
export const SeriesConfigSchema = z.object({
  data_key: z.string().describe('Column name from the data source to plot.'),
...
export const ColorScaleRuleSchema = z.object({
  ...
  minColor: z
    .string()
    .describe(
      'Color for the lowest value (overrides the derived tint). Hex, rgb(), rgba(), hsl(), hsla() or name.'
    )
    .optional(),
  maxColor: z
    .string()
  ...
  min: z
    .number()
    .describe('Explicit lower bound of the scale; defaults to the column minimum.')
    .optional(),
  max: z
    .number()
    .describe('Explicit upper bound of the scale; defaults to the column maximum.')
    .optional(),
})
...
export const StringRuleSchema = z.object({
  ...
  operator: z
    .enum(['equals', 'in', 'like'])
    .describe(
      '"equals": exact match; "in": value is one of a list; "like": case-insensitive substring.'
    ),
  value: z
...
export const ChartInputObjectSchema = z.object({
  source: ChartSourceSchema.describe(
    'Where the chart data comes from: a live Sim table read or static rows.'
  ),
...
export const TableInputSchema = z.object({
  source: ChartSourceSchema.describe(
    'Where the table data comes from: a live Sim table read or static rows.'
  ),
  chart_type: z.literal('table').describe('Display the result as a table.'),
...
const BaseInputSchema = z.object({
  source: ChartSourceSchema.describe('Where the visualization data comes from.'),
  ...
  x_axis_key: z
    .string()
    .describe('Column name for X-axis/category labels. Required for charts.')
    .optional(),
  x_axis_type: XAxisTypeEnum.nullable()
    .describe(
  ...
  title: z
    .string()
    .describe('A concise, descriptive title for the visualization. Required for charts.')
    .optional(),
...
export const InputSchema = BaseInputSchema.superRefine((input, context) => {
  ...
  // zod v4 : superRefine n'accepte que des issues custom — on reporte les
  // détails (message + chemin) au lieu de transférer l'objet issue tel quel.
