...
} from '@/providers/experiential/utils'
...
const logger = createLogger('ExperientialProvider')
...
async function applyResponseFormat(
  ...
  if (useNative) {
    logger.info('Using native structured outputs for Experiential model', { model })
  ...
  logger.info('Using json_object mode with prompt instructions for Experiential model', { model })
...
export const experientialProvider: ProviderConfig = {
  id: 'experiential',
  name: 'ExperientialLabs',
  description: 'Fast inference for open-source models via ExperientialLabs',
  version: '1.0.0',
  models: getProviderModels('experiential'),
  defaultModel: getProviderDefaultModel('experiential'),

  executeRequest: async (
    request: ProviderRequest
  ): Promise<ProviderResponse | StreamingExecution> => {
    if (!request.apiKey) {
      throw new Error('API key is required for ExperientialLabs')
    }
    ...
    const requestedModel = request.model.replace(/^experiential\//, '')

    logger.info('Preparing Experiential request', {
    ...
    const formattedMessages = formatMessagesForProvider(allMessages, 'experiential') as Message[]
    ...
    if (tools?.length) {
      preparedTools = prepareToolsWithUsageControl(tools, request.tools, logger, 'experiential')
    ...
    try {
      ...
      while (iterationCount < MAX_TOOL_ITERATIONS) {
        ...
        enrichLastModelSegmentFromChatCompletions(
          ...
          currentResponse,
          toolCallsInResponse,
          { model: request.model, provider: 'experiential' }
        )
        ...
        const toolExecutionPromises = toolCallsInResponse.map(async (toolCall) => {
          ...
          try {
            ...
            const toolCallEndTime = Date.now()
            logger.error('Error processing tool call (Experiential):', {
      ...
      logger.error('Error in Experiential request:', errorDetails)
      if (isAbortError(error) || request.abortSignal?.aborted) {
        throw error
      }
